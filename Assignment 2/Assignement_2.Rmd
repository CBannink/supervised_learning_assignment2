...
#Exploratory Data Analysis (EDA) To gain insights into the dataset, we
first performed exploratory data analysis. We created summary
statistics, distribution plots, and boxplots to understand the
distribution of quality ratings and the relationships between different
...
influences the quality of white wine based on visualization, we explored
the correlation matrix to understand the relationships between different
...
To better visualize the correlation between the features we construct a
correlogram.
...
In the following plots we investigate relationships between different
variables, and color points by quality for comparison.
...
In the common grading system of 1 to 10, a score below 6 would be seen
as negative and anything above 5 would be seen as positive. For a
classification job like this having many more datapoints in one group
compared to the other results in a prediction bias towards the group
with the most datapoints. Since our cut-off is between 5 and 6 the
majority of the wines are considered good (Sup.fig 1A). The problem with
this distribution is that when we train our model with this dataset,
that its predictions will be biased towards "good". Since we would like
to use this model to be sure no one will have to drink bad wines, we
would rather be more careful providing good scores than to be generous
in classifying a wine as good.

The two options we came up with are, #1 Keeping the cut-off score at
(<6) and duplicating the wines that are considered bad and use this to
train the model, or #2 Change the cut-off score to (<7). Option 1
results makes the model more careful to grade a wine as good and will be
more prone to score a wine as bad (Sup.fig 1B). Option 2 will make sure
that the number of datapoints in the two group (bad and good) will be of
similar sizes (Sup.fig 1C).
...
supp.fig1 <- ggarrange(Original, 
                       ggarrange(Option1, Option2, labels = c("B","C"), ncol=2), nrow = 2, labels = "A")
...
Then we divide the data into a Train and a Test set, for which the
proportion is 90% Train and 10% Test.
...
#Simple model 
We started by building a simple model using a decision
tree classifier. The dataset was split into training and testing sets,
and the model was trained using 10-fold cross-validation. Next to the 10-fold cross-validation, we performed a grid search for four different values of the complexity parameter on both datasets. 
...
```{r}
#rpart.plot(best_model_simple, main = "Decision Tree - Simple model")
```
...
With this confusion matrix we can plot a POC curve. As can be seen in
the plot the AUC is 0.735 for the simple model.
...
#Complex model 
For the improvement of the simpler model we use two random forest models, which we train with gridsearch to find the best
hyperparameter settings. The hyperparameter we optimized for is the total number of features taken into account with for each tree. We tried the total feature count minus one, half the total number of features, and the square root of the total number of features. 
...
#choose the best model with settings
We looked at the results for different mtry (number of features per tree in random forest) numbers and plotted the accuracy levels. As you can see, the different mtry levels did not influence the accuracy outcome much. This is caused by .........
```{r}
...
The varImpPlot function is part of the randomForest package and is used to create a variable importance plot based on the Gini index. The Gini index measures the impurity of the node, and the importance of a variable is computed based on how much it contributes to decreasing the impurity across all the trees in the random forest.

```{r}
#varImpPlot(best_model_duplicated, main = "Variable Importance Plot")
```
...
In conclusion, both models were able to predict wine quality to some
extent, with the complex model outperforming the simple model. The ROC
curves and AUC values indicate the trade-off between sensitivity and
specificity.
